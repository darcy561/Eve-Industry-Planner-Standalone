...
func SubscribeAdjustedPrices(js jetstream.JetStream, redisClient *redislib.Client, pool *antslib.Pool, natsConn *natslib.Conn) (func(context.Context), error) {
			...
			default:
				...
				for msg := range msgs.Messages() {
					...
					err := pool.Submit(func() {
						...
						esi.RefreshAdjustedPrices(wrapJetStreamMsg(jetstreamMsg), redisClient, natsConn)
					})
					if err != nil {
	...
	logs.Info("subscribed to adjusted prices refresh", "subject", subject, "consumer", consumerName, "type", "pull")
